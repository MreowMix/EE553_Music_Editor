...
int main(int argc, char** argv) {
   MidiFile newfile;        // create an empty MIDI file with one track
   newfile.absoluteTicks();  // time information stored as absolute time

   newfile.addTrack(2);     // Add another two tracks to the MIDI file
   vector<uchar> midievent;     // storage for MIDI events
   ...
   int i=0;
   int actiontime = 0;      // storage for MIDI event time
   ...
   while (melody[i] >= 0) {
      ...
      newfile.addEvent(1, actiontime, midievent);
   ...
   while (bass[i] >= 0) {
      midievent[0] = 0x90;
      midievent[1] = bass[i];
      newfile.addEvent(2, actiontime, midievent);
   ...
   newfile.sortTracks();         // make sure data is in correct order
   newfile.write("twinkle.mid"); // write Standard MIDI File twinkle.mid
   return 0;
}
