...
public:
      MidifileREAD(const char* file,int file_size)throw(bad_alloc):size(file_size),buffer(new char[size]){
...
public:
    Header(const char* f,int s)throw(bad_alloc):MidifileREAD(f,s),ff_ff(new unsigned char[2]),nn_nn(new unsigned char[2]),dd_dd(new unsigned char[2]){
...
public:
    Track(const char* f,int s)throw(bad_alloc):Header(f,s),track_length(new unsigned char[4]){
        vector<unsigned char> Track;
        Track=Getdata();
...
public:
    ...
    Seperate_Event(const char* f,int s)throw(bad_alloc):Track(f,s){
